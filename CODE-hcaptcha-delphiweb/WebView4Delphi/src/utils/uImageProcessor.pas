...
begin
  ...
  try
    // Load source image
    ImageFormat := GetImageFormat(SourceStream);
...
var
  ...
  ImageFormat, LowerTargetFormat: string;
begin
  SourceStream := Base64ToStream(Base64Image);
  SourceBitmap := TBitmap.Create;
  ResultStream := TMemoryStream.Create;
  try
    ...
    LowerTargetFormat := LowerCase(TargetFormat);
    if LowerTargetFormat = 'png' then
    begin
      PngImage := TPngImage.Create;
      try
    ...
    else if (LowerTargetFormat = 'jpeg') or (LowerTargetFormat = 'jpg') then
    begin
