...
type
  // Loader events
  TLoaderNotifyEvent = procedure(Sender: TObject) of object;
  TLoaderBrowserProcessExitedEvent = procedure(Sender: TObject; const aEnvironment: ICoreWebView2Environment; const aArgs: ICoreWebView2BrowserProcessExitedEventArgs) of object;
  TLoaderNewBrowserVersionAvailableEvent = procedure(Sender: TObject; const aEnvironment: ICoreWebView2Environment) of object;
  TLoaderProcessInfosChangedEvent = procedure(Sender: TObject; const aEnvironment: ICoreWebView2Environment) of object;
  TLoaderGetCustomSchemesEvent = procedure(Sender: TObject; var aCustomSchemes: TWVCustomSchemeInfoArray) of object;

  // Interface que define eventos do loader
  IWVLoaderEvents = interface
    ['{5B91E1CD-CA14-4667-9D8D-5D39749B2808}']
    procedure doOnInitializationError; 
    procedure doOnEnvironmentCreated;
    procedure doOnNewBrowserVersionAvailable(const aEnvironment: ICoreWebView2Environment);
    procedure doOnBrowserProcessExitedEvent(const sender: ICoreWebView2Environment; const args: ICoreWebView2BrowserProcessExitedEventArgs);
    procedure doProcessInfosChangedEvent(const sender: ICoreWebView2Environment);
    procedure doOnGetCustomSchemes(var aSchemeRegistrations: TWVCustomSchemeInfoArray);
  end;
